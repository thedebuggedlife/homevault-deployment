...
print_usage() {
    if [ -z "$SELECTED_ACTION" ]; then
        ...
        echo "                                  Format: '[#h][#H][#d][#D][#w]|[#W][#m][#M][#y][#Y]' Where:"
        echo "                                    #(h|d|w|m|y) keep only the most recent (hourly|daily|weekly|monthly|yearly)"
        echo "                                                 snapshot for the last #(hours|days|weeks|months|years)"
        echo "                                    #(H|D|W|M|Y) keep all (hourly|daily|weekly|monthly|yearly) snapshots"
        echo "                                                 for the last #(hours|days|weeks|months|years)"
        echo "                                  Default: 7D4W16M10Y"
        echo "                                  Examples:"
        echo "                                    --retention 24H7D keep all hourly snapshots for the past day"
        echo "                                                      and all daily snapshots for the past week"
        echo "                                    --retention 7d    keep only most recent daily snapshot for the last week"
